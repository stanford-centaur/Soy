...
Equation::Equation() : _scalar(0), _type(Equation::EQ) {}

Equation::Equation(EquationType type) : _scalar(0), _type(type) {}

void Equation::addAddend(double coefficient, unsigned variable) {
  _addends.append(Addend(coefficient, variable));
}

void Equation::setScalar(double scalar) { _scalar = scalar; }

void Equation::setType(EquationType type) { _type = type; }
...
void Equation::updateVariableIndex(unsigned oldVar, unsigned newVar) {
  ...
  if (oldVarIt == _addends.end()) return;
...
bool Equation::equivalent(const Equation &other) const {
  if (_scalar != other._scalar) return false;
...
void Equation::dump(String &output) const {
  output = "";
  for (const auto &addend : _addends) {
    if (FloatUtils::isZero(addend._coefficient)) continue;

    if (FloatUtils::isPositive(addend._coefficient)) output += String("+");
...
bool Equation::isVariableMergingEquation(unsigned &x1, unsigned &x2) const {
  if (_addends.size() != 2) return false;

  if (_type != Equation::EQ) return false;

  if (!FloatUtils::isZero(_scalar)) return false;
...
Set<unsigned> Equation::getParticipatingVariables() const {
  Set<unsigned> result;
  for (const auto &addend : _addends) result.insert(addend._variable);
...
List<unsigned> Equation::getListParticipatingVariables() const {
  ...
  for (const auto &addend : _addends) result.append(addend._variable);
...
double Equation::getCoefficient(unsigned variable) const {
  for (const auto &addend : _addends) {
    if (addend._variable == variable) return addend._coefficient;
  }
