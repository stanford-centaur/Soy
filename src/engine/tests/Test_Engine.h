...
class MockForEngine : public MockTableauFactory,
...
class EngineTestSuite : public CxxTest::TestSuite {
 public:
  ...
  void test_process_input_query() {
    ...
    inputQuery.addPLConstraint(relu1);
    inputQuery.addPLConstraint(relu2);
  ...
  void test_pick_split_pl_constraint_polarity() {
    ...
    // And the picked ReLU must have |polarity| not larger than the other
    // unfixed ReLUs
    ...
    inputQuery.addPLConstraint(relu1);
    inputQuery.addPLConstraint(relu2);
    inputQuery.addPLConstraint(relu3);
    inputQuery.addPLConstraint(relu4);
    ...
    PLConstraint *constraintToSplit;
    PLConstraint *constraintToSplitSnC;
    constraintToSplit = engine.pickSplitPLConstraint(DivideStrategy::Polarity);
    constraintToSplitSnC =
        engine.pickSplitPLConstraintSnC(SnCDivideStrategy::Polarity);
    ...
    for (const auto &constraint : engine.getInputQuery()
      ...
      TS_ASSERT(
          std::abs(((ReluConstraint *)constraintToSplit)->computePolarity()) <=
          std::abs(((ReluConstraint *)constraint)->computePolarity()));
    }

    TS_ASSERT_EQUALS(0,
                     ((ReluConstraint *)constraintToSplit)->computePolarity());
  }
  ...
  void test_pick_split_pl_constraint_earliest_relu() {
    ...
    PLConstraint *constraintToSplitSnC;
    constraintToSplit =
        engine.pickSplitPLConstraint(DivideStrategy::EarliestReLU);
    constraintToSplitSnC =
        engine.pickSplitPLConstraintSnC(SnCDivideStrategy::EarliestReLU);
    TS_ASSERT_EQUALS(constraintToSplitSnC, constraintToSplit);

    PLConstraint *firstConstraintInTopologicalOrder;
  ...
  void test_pick_split_pl_constraint_largest_interval() {
    ...
    PLConstraint *constraintToSplit;
    constraintToSplit =
        engine.pickSplitPLConstraint(DivideStrategy::LargestInterval);
    ...
    List<PiecewiseLinearCaseSplit> caseSplits =
        constraintToSplit->getCaseSplits();
  ...
  void test_todo() {
    TS_TRACE(
        "Future work: Guarantee correct behavior even when some variable "
        "is unbounded\n");
  }
};
