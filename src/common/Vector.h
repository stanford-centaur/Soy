...
class Vector {
 ...
 public:
  ...
  virtual void append(T value) { _container.push_back(value); }
  ...
  T get(int index) const { return _container.at(index); }

  T &operator[](int index) { return _container[index]; }
  ...
  void eraseAt(unsigned index) {
    if (index >= size()) throw CommonError(CommonError::VECTOR_OUT_OF_BOUNDS);
  ...
  Vector<T> operator+(const Vector<T> &other) {
    ...
    for (unsigned i = 0; i < this->size(); ++i) output.append((*this)[i]);

    for (unsigned i = 0; i < other.size(); ++i) output.append(other.get(i));
  ...
  T popFirst() {
    if (size() == 0) throw CommonError(CommonError::POPPING_FROM_EMPTY_VECTOR);
  ...
  T first() const {
    if (empty()) throw CommonError(CommonError::VECTOR_OUT_OF_BOUNDS);
  ...
  bool operator==(const Vector<T> &other) const {
    ...
    for (unsigned i = 0; i < size(); ++i) {
      if (!copyOfOther.exists(get(i))) return false;
  ...
  static void cartesian(const Vector<Vector<T>> &v,
  ...
  void sort() { std::sort(_container.begin(), _container.end()); }

  Super getContainer() const { return _container; }
