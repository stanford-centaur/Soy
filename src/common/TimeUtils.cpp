...
struct timespec TimeUtils::sampleMicro() {
  ...
  now.tv_nsec = ((count.QuadPart % gCountsPerSec.QuadPart) *
...
unsigned long long TimeUtils::timePassed(const struct timespec &then,
  ...
  unsigned long long secondsAsMicro =
      (now.tv_sec - then.tv_sec) * MICROSECONDS_IN_SECOND;
  unsigned long long nanoAsMicro =
      (now.tv_nsec - then.tv_nsec) / NANOSECONDS_IN_MICROSECOND;
...
String TimeUtils::now() {
  ...
  return Stringf("%02u:%02u:%02u", formattedTime->tm_hour,
                 formattedTime->tm_min, formattedTime->tm_sec);
}
