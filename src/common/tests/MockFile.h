...
class MockFile : public IFile {
 public:
  ...
  String writtenLines;
  void write(const String &line) { writtenLines += line; }

  String readLine(char lineSeparatingChar) {
    Stringf separatorAsString("%c", lineSeparatingChar);

    String result =
        writtenLines.substring(0, writtenLines.find(separatorAsString));
    writtenLines = writtenLines.substring(
        writtenLines.find(separatorAsString) + 1, writtenLines.length());
    return result;
  }
