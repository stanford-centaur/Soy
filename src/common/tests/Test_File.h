...
class MockForFile : public RealMalloc,
                    public MockErrno,
                    public T::Base_open,
                    public T::Base_write,
                    public T::Base_read,
 ...
 public:
  ...
  ssize_t read(int fd, void *buf, size_t count) {
    ...
    HeapData tempData = ConstSimpleData((char *)(nextReadData.data()) + bytes,
                                        nextReadData.size() - bytes);
...
class FileTestSuite : public CxxTest::TestSuite {
 public:
  ...
  void setUp() { TS_ASSERT(mock = new MockForFile); }
  ...
  void xtest_write() {
    ...
    TS_ASSERT_SAME_DATA(mock->writtenData.data(), expected.ascii(),
                        expected.length());
  }
  ...
  void xtest_write__fails() {
    ...
    TS_ASSERT_THROWS_EQUALS(file.write(line1), const CommonError &e,
                            e.getCode(), CommonError::WRITE_FAILED);
  }
  ...
  void xtest_read__fails() {
    ...
    TS_ASSERT_THROWS_EQUALS(file.read(readData1, 5), const CommonError &e,
                            e.getCode(), CommonError::READ_FAILED);
  }
  ...
  void xtest_readline() {
    ...
    Stringf all("%s%s%s%s%s%s", toRead1, toRead2, toRead3, toRead4, toRead5,
                toRead6);
    ...
    TS_ASSERT_THROWS_EQUALS(file.readLine(), const CommonError &e, e.getCode(),
  ...
  void xtest_size() {
    ...
    TS_ASSERT_THROWS_EQUALS(File::getSize("test/test.a"), const CommonError &e,
                            e.getCode(), CommonError::STAT_FAILED);
  }
