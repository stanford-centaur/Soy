...
class List {
 ...
 public:
  ...
  List<T>(InputIt begin, InputIt end) : _container(begin, end) {}

  void append(const List<T> &other) {
    for (const auto &element : other) _container.push_back(element);
  }

  void append(const T &value) { _container.push_back(value); }

  void appendHead(const T &value) { _container.push_front(value); }
  ...
  void popBack() {
    if (empty()) throw CommonError(CommonError::LIST_IS_EMPTY);
