...
class HashSet {
 ...
 public:
  ...
  void insert(const Value &value) { _container.insert(value); }
  ...
  void operator+=(const HashSet<Value> &other) { insert(other); }
  ...
  void erase(const Value &value) { _container.erase(value); }

  const Super &container() const { return _container; }
