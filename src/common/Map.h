...
class Map {
 ...
 public:
  ...
  Map<Value, List<Key>> flip() {
    ...
    for (const auto &pair : _container) result[pair.second].append(pair.first);

    return result;
  }

  Value &operator[](const Key &key) { return _container[key]; }

  Set<Value> values() const {
    Set<Value> result;
    for (auto it : _container) result.insert(it.second);
  ...
  Set<Key> keys() const {
    Set<Key> result;
    for (auto it : _container) result.insert(it.first);

    return result;
  }

  const Value &operator[](const Key &key) const { return _container.at(key); }

  const Value &at(const Key &key) const {
    if (!exists(key)) throw CommonError(CommonError::KEY_DOESNT_EXIST_IN_MAP);
  ...
  iterator erase(iterator it) { return _container.erase(it); }
  ...
  void setIfDoesNotExist(Key key, const Value &value) {
    if (!exists(key)) _container[key] = value;
  }

  Key keyWithLargestValue() const {
    if (empty()) throw CommonError(CommonError::MAP_IS_EMPTY);

    Key maxKey = _container.begin()->first;

    for (const auto &pair : _container)
      if (pair.second > _container.at(maxKey)) maxKey = pair.first;
