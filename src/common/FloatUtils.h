...
class FloatUtils {
 public:
  ...
  static String doubleToString(
      double x, unsigned precision =
                    GlobalConfiguration::DEFAULT_DOUBLE_TO_STRING_PRECISION);
  ...
  static bool isInteger(
      double x,
      double epsilon =
          GlobalConfiguration::DEFAULT_EPSILON_FOR_INTEGRAL_COMPARISONS) {
    ASSERT(epsilon > 0);
    return areEqual(x, round(x), epsilon);
  }

  static bool areEqualInt(
      double x, double y,
      double epsilon =
          GlobalConfiguration::DEFAULT_EPSILON_FOR_INTEGRAL_COMPARISONS) {
    ASSERT(epsilon > 0);
  ...
  static double roundUp(double x) { return isInteger(x) ? round(x) : ceil(x); }

  static double roundDown(double x) {
    return isInteger(x) ? round(x) : floor(x);
  }
  ...
  static double infinity() { return DBL_MAX; }

  static double negativeInfinity() { return -DBL_MAX; }

  static bool isFinite(double x) {
    return (x != infinity()) && (x != negativeInfinity());
  }

  static bool wellFormed(double x) { return !isNan(x) && !isInf(x); }
};
