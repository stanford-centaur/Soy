...
File::~File() { closeIfNeeded(); }

void File::close() { closeIfNeeded(); }

bool File::directory(const String &path) {
  struct stat fileData;
  memset(&fileData, 0, sizeof(struct stat));

  if (T::stat(path.ascii(), &fileData) == 0) return S_ISDIR(fileData.st_mode);
...
void File::open(Mode openMode) {
  ...
  if ((openMode == File::MODE_WRITE_APPEND) ||
      (openMode == File::MODE_WRITE_TRUNCATE))
...
String File::readLine(char lineSeparatingChar) {
  ...
  String result =
      _readLineBuffer.substring(0, _readLineBuffer.find(separatorAsString));
  _readLineBuffer = _readLineBuffer.substring(
      _readLineBuffer.find(separatorAsString) + 1, _readLineBuffer.length());
  return result;
}
